...
import { Activity, ActivityId, ActivityType, AllBranchData, Branch, BranchData, ConcreteActivity } from './branch';
import { Db } from './db';
...
export function selectTrailmen(names: string[], quiet = false): number {
  ...
  if (!quiet && messages.length) Dialog.textarea(messages.join('\n\n'));
  return ids.length;
};
...
export function scrapeBranch(): BranchData {
  ...
  if (!v.is(Branch, branch)) return {branch: '', activities: {}, needCoreSteps: 0, needElectives: 0};
